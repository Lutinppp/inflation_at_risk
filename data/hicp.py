...
Returns [iso3, year, hicp] for EU member states + UK + Norway.
...
def _fetch_eurostat_hicp() -> pd.DataFrame | None:
    ...
    SDMX 3.0 API (csvdata / formatVersion 2.0), then average within each
    calendar year so that partial years still produce a valid annual estimate.

    Returns DataFrame with [iso3, year, hicp] or None on failure.
    """
    ...
    # Average all available monthly observations per (country, year)
    # This naturally handles partial years without special-casing
    annual = (
...
def fetch_hicp(save: bool = True) -> pd.DataFrame:
    """
    Fetch annual HICP % change for EU + UK + Norway.
    Primary: Eurostat REST API.  Fallback: compiled historical table.

    Returns DataFrame: iso3, year, hicp (percentage points).
    """
    print("  Fetching Eurostat HICP annual % change (prc_hicp_minr, RCH_A) …")

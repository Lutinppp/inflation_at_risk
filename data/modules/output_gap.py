...
def _fetch_oecd_output_gap() -> pd.DataFrame | None:
    """
    Fetch output gap (% of potential GDP, measure GAP, quarterly) from the OECD
    Economic Outlook SDMX-JSON API.  No API key is required.

    Returns long DataFrame [iso3, year, quarter, output_gap] or None on failure.
    ...
    try:
        ...
        # Quarterly time periods: strings like "2022-Q3"
        period_map = {
            str(i): v["id"]
        ...
        for t_idx, vals in sdata.get("observations", {}).items():
            ...
            if period and vals[0] is not None:
                try:
                    ...
                    continue
                records.append(
                    {"iso3": iso3, "year": year, "quarter": quarter, "output_gap": vals[0]}
                )
...
def _compiled_fallback() -> pd.DataFrame:
    ...
    for iso3, yr_data in og_data.items():
        for year, val in yr_data.items():
            records.append({"iso3": iso3, "year": year, "quarter": 4, "output_gap": val})
...
def fetch_output_gap(save: bool = True) -> pd.DataFrame:
    ...
    Returns DataFrame: iso3, year, quarter, output_gap (%).
    """
    ...
    if df is None or len(df) < 50:
        ...
        combined = combined.drop_duplicates(subset=["iso3", "year", "quarter"], keep="first")
        df = combined

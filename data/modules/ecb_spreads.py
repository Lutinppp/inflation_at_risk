...
Spread = country yield − Germany (Bund) yield, monthly.
...
def _fetch_eurostat_yields() -> pd.DataFrame | None:
    ...
    and keep native monthly frequency.
    Returns DataFrame with columns [iso3, year, month, yield_10y, spread_10y],
    or None on failure.
    """
    params = {
        "format":      "SDMX-CSV",
    ...
    df["month"] = pd.to_numeric(df[time_col].astype(str).str[5:7], errors="coerce")
    df["yield_10y"] = pd.to_numeric(df[val_col], errors="coerce")
    df = df.dropna(subset=["iso3", "year", "month", "yield_10y"])
    df["year"] = df["year"].astype(int)
    df["month"] = df["month"].astype(int)

    # Spread = country yield − Bund in the same month.
    bund = (
        df[df["iso3"] == "DEU"][ ["year", "month", "yield_10y"] ]
        .rename(columns={"yield_10y": "bund_yield_10y"})
    )
    df = df.merge(bund, on=["year", "month"], how="left")
    df["spread_10y"] = df["yield_10y"] - df["bund_yield_10y"]

    return (
        df[["iso3", "year", "month", "yield_10y", "spread_10y"]]
...
def _compiled_fallback() -> pd.DataFrame:
    ...
    for iso3, yr_data in yields.items():
        for year, yld in yr_data.items():
            ...
            records.append({"iso3": iso3, "year": year, "month": 12, "yield_10y": yld, "spread_10y": spread})
...
def fetch_spreads(save: bool = True) -> pd.DataFrame:
    ...
    Returns DataFrame: iso3, year, month, yield_10y, spread_10y (percentage points).
    """
    print("  Fetching Eurostat 10Y sovereign yields (IRT_LT_MCBY_M, monthly) for all EU panel countries …")

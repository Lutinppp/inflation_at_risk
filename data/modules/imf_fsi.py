...
def fetch_fsi(save=True):
   """Fetch ECB CLIFS financial stress index for EU-27 + UK.
    Returns long DataFrame with columns [iso3, year, month, clifs].
   ...
   for cc, iso3 in ECB_TO_ISO3.items():
       ...
       try:
           ...
           monthly["iso3"] = iso3
           records.append(monthly[["iso3", "year", "month", "clifs"]])
           print(f"  {cc} ({iso3}): {monthly['clifs'].notna().sum()} monthly obs")
       except Exception as e:
           print(f"  WARNING: {cc} failed — {e}")
           continue
   if not records:
       raise RuntimeError("CLIFS fetch returned no data for any country.")
   out = pd.concat(records, ignore_index=True).sort_values(["iso3", "year", "month"])

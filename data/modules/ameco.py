...
Returns [iso3, year, quarter, infl_expectations].
...
_OECD_MEASURE    = "CPIH_YTYPCT"   # HICP y/y % change, forecast + actuals
_OECD_FREQ       = "Q"
...
def _fetch_oecd_cpih() -> pd.DataFrame | None:
    ...
    Returns long DataFrame [iso3, year, quarter, infl_expectations] or None on failure.
    """
    ...
    return df.sort_values(["iso3", "year", "quarter"]).reset_index(drop=True)
...
def _fetch_ecb_ces() -> pd.DataFrame | None:
    ...
    Returns long DataFrame [iso3, year, month, infl_expectations] or None on failure.
    ...
    for key in _CES_SERIES_CANDIDATES:
        ...
        try:
            ...
            # Extract year/month from YYYY-MM time period
            df["year"] = df["time_period"].str[:4]
            df["month"] = df["time_period"].str[5:7]
            ...
            out = (
                df[["iso3", "year", "month", "value"]]
                .rename(columns={"value": "infl_expectations"})
                .sort_values(["iso3", "year", "month"])
                .reset_index(drop=True)
            )
            print(f"  ECB CES loaded: {len(out)} country-month obs (key: {key})")
            return out
...
def _compiled_fallback() -> pd.DataFrame:
    ...
    for iso3, yr_data in ameco_data.items():
        for year, val in yr_data.items():
            records.append({"iso3": iso3, "year": year, "quarter": 4, "infl_expectations": val})
...
def fetch_ameco(save: bool = True) -> pd.DataFrame:
    ...
    1. OECD EO CPIH_YTYPCT (quarterly): primary source.
    2. Compiled fallback: supplements gaps where needed (encoded at Q4).

    Returns DataFrame: iso3, year, quarter, infl_expectations (% y/y).
    """
    ...
    if base_df is None or base_df.empty:
        ...
        if not cyp_mlt.empty:
            base_df = pd.concat([base_df, cyp_mlt], ignore_index=True) \
                        .drop_duplicates(subset=["iso3", "year", "quarter"], keep="first")
    ...
    sample_2024 = (
        df[df["year"] == 2024]
        .groupby("iso3")["infl_expectations"]
        .mean()
    )

...
Returns [iso3, year, import_price_chg] for all available EU+ panel countries.
...
def _compiled_fallback() -> pd.DataFrame:
    ...
    for iso3 in EU_PANEL_COUNTRIES:
        ...
        for year, val in yr_data.items():
            if not np.isnan(val):
                records.append({"iso3": iso3, "year": year, "import_price_chg": val})
...
def fetch_import_prices(save: bool = True) -> pd.DataFrame:
    """
    Fetch annual growth rate of imports of goods & services for EU panel countries.
    Primary: ECB MNA (MNA.A.N.*.W1.S1.S1.C.P7._Z._Z._Z.*.LR.GY).
    Fallback: compiled table.

    Returns DataFrame: iso3, year, import_price_chg (%).
    """
    print("  Fetching ECB MNA imports growth rate (P7, LR, GY) …")
    df_live = _fetch_ecb_imports()

    if df_live is not None and len(df_live) >= 100:
        n_countries = df_live["iso3"].nunique()
        print(f"  ECB MNA imports: {len(df_live)} obs, {n_countries} countries")
        ...
        print("  ECB MNA unavailable or insufficient — using compiled fallback.")
